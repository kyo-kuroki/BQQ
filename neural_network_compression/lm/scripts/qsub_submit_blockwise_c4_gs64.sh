#!/bin/bash
# Submit block-wise BQQ quantization jobs: c4, gs=64, 30000 steps.
# Output directories use suffix -gs64-30000step-c4 to avoid overwriting gs32 results.
#
# Usage:
#   bash qsub_submit_blockwise_c4_gs64.sh              # submit all 6 combinations
#   bash qsub_submit_blockwise_c4_gs64.sh 2B 3         # submit only 2B 3-bit
#   bash qsub_submit_blockwise_c4_gs64.sh 9B           # submit 9B 2-bit and 3-bit
...
submit_blockwise() {
    ...
    local LOG_DIR="${SCRIPT_DIR}/qsub_jobs/blockwise-${MODEL_TAG}-bit${BIT_WIDTH}-gs64/logs"
    local JOB_NAME="bw64_${MODEL_TAG}_${BIT_WIDTH}b"
