#!/bin/bash
# Submit block-wise BQQ quantization jobs: c4, gs=32, 30000 steps, forward order (no reverse).
#
# Usage:
#   bash qsub_submit_blockwise_c4_gs32.sh              # submit all 6 combinations
#   bash qsub_submit_blockwise_c4_gs32.sh 2B 3         # submit only 2B 3-bit
#   bash qsub_submit_blockwise_c4_gs32.sh 9B           # submit 9B 2-bit and 3-bit
...
submit_blockwise() {
    ...
    local LOG_DIR="${SCRIPT_DIR}/qsub_jobs/blockwise-${MODEL_TAG}-bit${BIT_WIDTH}-gs32/logs"
    local JOB_NAME="bw32_${MODEL_TAG}_${BIT_WIDTH}b"
    ...
    if [[ "${DONE}" -eq "${N_BLOCKS}" ]]; then
        echo "[${MODEL_TAG} ${BIT_WIDTH}-bit gs32] All ${N_BLOCKS} blocks done. Skipping."
        return
    fi

    echo "[${MODEL_TAG} ${BIT_WIDTH}-bit gs32] Submitting ${N_BLOCKS} tasks (${DONE}/${N_BLOCKS} already done)"
