#!/bin/bash
# Submit block-wise BQQ quantization jobs using RedPajama-1T calibration data.
# Saves to a separate directory (suffix: -redpajama1t) to avoid overwriting existing results.
#
# Usage:
#   bash qsub_submit_blockwise_redpajama.sh              # submit all 6 combinations
#   bash qsub_submit_blockwise_redpajama.sh 2B 3         # submit only 2B 3-bit
#   bash qsub_submit_blockwise_redpajama.sh 9B           # submit 9B 2-bit and 3-bit
...
submit_blockwise() {
    ...
    local LOG_DIR="${SCRIPT_DIR}/qsub_jobs/blockwise-${MODEL_TAG}-bit${BIT_WIDTH}-${DATASET}/logs"
    local JOB_NAME="bw_${MODEL_TAG}_${BIT_WIDTH}b_rp"
    ...
    if [[ "${DONE}" -eq "${N_BLOCKS}" ]]; then
        echo "[${MODEL_TAG} ${BIT_WIDTH}-bit ${DATASET}] All ${N_BLOCKS} blocks done. Skipping."
        return
    fi

    echo "[${MODEL_TAG} ${BIT_WIDTH}-bit ${DATASET}] Submitting ${N_BLOCKS} tasks (${DONE}/${N_BLOCKS} already done)"
