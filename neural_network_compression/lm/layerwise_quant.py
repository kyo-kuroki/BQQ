...
def _quantize_block_worker(rank: int, gpu_tasks: list, common: dict):
    ...
    for task in tasks:
        ...
        reconstructed = quantizer.bqq_large_matrix_multi_worker(
            ...
            importance_weight=(str(common.get('bqq_opt_mode', 'plain')).lower() in ('activation-aware', 'fullchol')),
            importance_score='fullchol',
            diag_power=common.get('diag_power', 1.0),
    ...
    *,
    ...
    for module_name, weight in work_items.items():
        ...
        reconstructed = quantizer.bqq_large_matrix_multi_worker(
            ...
            importance_weight=(str(bqq_opt_mode).lower() in ('activation-aware','fullchol')),
            importance_score='fullchol',
    ...
    *,
    ...
    for name, param in model.named_parameters():
        if is_quantization_target(name) and param.ndim == 2:
            if layer_threshold > 0 and not passes_layer_threshold(name, layer_threshold):
                continue
            target_names.append(name[:-len('.weight')])
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='wikitext2',
                        choices=['wikitext2', 'ptb', 'c4', 'slimpajama'])
    ...
    # target. No model load and no calibration data needed (so a fleet of these
    # processes can run in parallel across GPUs). Assembly is done separately.
