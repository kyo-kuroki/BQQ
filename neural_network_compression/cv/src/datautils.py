"""ImageNet / CIFAR data loading utilities for CV BQQ calibration and evaluation.

Parallels lm/src/datautils.py (the calibration/evaluation data layer), but for
vision models: calibration samples are images, not token IDs.
"""
...
def build_transform(input_size=224, interpolation="bicubic",
    ...
    if resize_im:
        ...
        t.append(transforms.Resize(size, interpolation=ip))  # maintain ratio w.r.t. 224 images
...
def _model_normalization(model_name):
    ...
    raise NotImplementedError(f"Unknown model family for normalization: {model_name}")


def get_imagenet(model_name, val_batchsize=32, calib_batchsize=32, num_workers=16,
                 num_traindatas=None, data_path=None, seed=0):
    data_path = _resolve_imagenet_path(data_path)

    mean, std, crop_pct = _model_normalization(model_name)
...
def get_cifar_dataloader(dataset_name, batch_size=64):
    transform = transforms.Compose([
        transforms.Resize((224, 224)),  # resize CIFAR images to the model input size
        transforms.ToTensor(),
