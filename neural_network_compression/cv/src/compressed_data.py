...
NN_COMPRESSION_DIR = CV_DIR.parent
...
def default_compressed_data_dir(model_name: str, bit_width: int, group_size: int, num_steps: int) -> Path:
    return CV_DIR / "bqq_compressed_data" / f"{model_basename(model_name)}-{bit_width}bit-{group_size}gs-{num_steps}step"


def default_quantized_model_dir(model_name: str) -> Path:
    return CV_DIR / "quantized_models" / model_basename(model_name)


def default_results_dir() -> Path:
    return CV_DIR / "results"


def default_hessian_cache_dir(model_name: str, nsamples: int, layer_threshold: int = 0) -> Path:
    return CV_DIR / "hessian_cache" / model_basename(model_name) / f"{nsamples}samples_thr{layer_threshold}"
...
def build_patch_index(weights_dir: str | Path) -> dict[str, list[Path]]:
    ...
    # os.scandir is far faster than glob("*.pth") + sorted() (handles millions of files)
...
def consolidated_path(weights_dir: Path, layer_name: str) -> Path:
    """Return the path of the consolidated patch file for a layer."""
...
def consolidate_target_patches(
    ...
    *,
    ...
    """Consolidate a single target's patch files into _consolidated/.

    Enumerates (i, j) from the patch layout (shape, group_size), loads the
    matching _row{i}_col{j}.pth files, and merges them into one file. No full
    directory scan is needed, so this stays fast even with millions of files.
    """
...
def consolidate_all_patches(weights_dir: str | Path) -> None:
    """Consolidate every target's patch files into _consolidated/.

    Existing consolidated files are skipped. This turns replace_linear_with_bqq's
    millions of torch.load calls into one per target.
    """
    weights_dir = Path(weights_dir)
    out_dir = weights_dir / "_consolidated"
    out_dir.mkdir(exist_ok=True)
...
def build_consolidated_index(weights_dir: str | Path) -> dict[str, Path]:
    """Build a target-name -> consolidated-file-path dict from _consolidated/."""
...
def load_layer_patches(
    ...
    # consolidated index: value is a Path (single file)
    if isinstance(entry, Path):
        if entry.exists():
            return torch.load(entry, weights_only=False, map_location=map_location)
        return []

    # legacy patch index: value is list[Path]
    paths = entry
    if paths:
    ...
    # fallback: load individual patch files
    patch_list: list[dict] = []
    for path in paths:
