...
- Fast / mild accuracy: `./run_scripts/run_lm_layerwise_quant.sh`
- Medium speed / medium accuracy: `./run_scripts/run_lm_blockwise_quant.sh`
- Slow / high accuracy: `./run_scripts/run_lm_fine_tuning.sh` after a quantized model has been built
...
./run_scripts/run_lm_layerwise_quant.sh

# Medium: blockwise quantization/tuning.
./run_scripts/run_lm_blockwise_quant.sh

# Slow: fine-tune an assembled quantized model.
./run_scripts/run_lm_fine_tuning.sh
```

`./run_scripts/run_lm_layerwise_quant.sh` assigns one transformer block to each GPU job, collects all Hessians for that block once, then quantizes the block's Linear layers in parallel with `LAYERS_PER_GPU`.

`./run_scripts/run_lm_blockwise_quant.sh` assigns transformer blocks to GPU jobs and performs blockwise quantization/tuning. The current default is `PROGRESSIVE=1` with `PROGRESSIVE_MODE=layer-tune`; set `PROGRESSIVE=0` to use the older layerwise-first then block-tuning flow.

`./run_scripts/run_lm_fine_tuning.sh` starts from an assembled quantized model and performs STE fine-tuning.

Useful overrides:

```bash
MODEL_NAME=Qwen/Qwen3.5-2B BIT_WIDTH=2 GROUP_SIZE=64 ./run_scripts/run_lm_layerwise_quant.sh
MODEL_NAME=Qwen/Qwen3.5-2B BIT_WIDTH=2 GROUP_SIZE=64 ./run_scripts/run_lm_blockwise_quant.sh
MODEL_NAME=Qwen/Qwen3.5-2B MODEL_PATH=/path/to/quantized.pth ./run_scripts/run_lm_fine_tuning.sh

# Select GPUs and parallelism.
GPU_IDS=0,1,2,3 LAYERS_PER_GPU=4 ./run_scripts/run_lm_layerwise_quant.sh
GPU_IDS=0,1,2,3 BLOCKS_PER_GPU=1 ./run_scripts/run_lm_blockwise_quant.sh

# Process one block manually.
BLOCK_IDX=0 ./run_scripts/run_lm_layerwise_quant.sh
BLOCK_IDX=0 ./run_scripts/run_lm_blockwise_quant.sh
```
...
1. `./run_scripts/run_lm_layerwise_quant.sh` for the fastest layerwise-only quantized model
2. `./run_scripts/run_lm_blockwise_quant.sh` for blockwise quantization/tuning
3. `./run_scripts/run_lm_fine_tuning.sh` to improve an already assembled quantized model
...
`./run_scripts/run_lm_layerwise_quant.sh` variables:
...
`./run_scripts/run_lm_blockwise_quant.sh` variables:
...
`./run_scripts/run_lm_fine_tuning.sh` variables:
...
Standard fine-tuning is wrapped by `./run_scripts/run_lm_fine_tuning.sh`.
