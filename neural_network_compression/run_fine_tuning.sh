...
MODEL_NAME="${MODEL_NAME:-Qwen/Qwen3-2B}"
...
python "${LM_DIR}/fine_tuning.py"     --model_name "${MODEL_NAME}"     --model_path "${MODEL_PATH}"     --bit_width "${BIT_WIDTH}"     --group_size "${GROUP_SIZE}"     --num_steps "${FINETUNE_NUM_STEPS}"     --output_dir "${OUTPUT_DIR}"     --num_train_epochs "${FINETUNE_EPOCHS}"     --learning_rate "${FINETUNE_LR}"     --gradient_accumulation_steps "${GRADIENT_ACCUMULATION_STEPS}"     --max_seq_length "${MAX_SEQ_LENGTH}"     "$@"
